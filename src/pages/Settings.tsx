...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    const loadUserSettings = async () => {
      try {
        setIsLoading(true);
        setError(null);
        console.log('[Settings] Loading user settings from backend...');
        ...
        console.log('[Settings] Settings loaded:', {
          ...
          mistralTokenLength: data?.mistralToken?.length || 0,
  ...
  const handleMistralTokenChange = async (token: string) => {
    console.log('[Settings] Saving Mistral token, length:', token?.length || 0);
    setMistralToken(token);
    try {
      const updated = await userSettingsApi.update({
        mistralToken: token || null
      });
      console.log('[Settings] Mistral token saved, response has token:', !!updated?.mistralToken);
      setSettings(updated);
    } catch (err: any) {
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center sm:flex-row sm:items-center py-3 sm:py-4 px-2 sm:px-1 border-b-2 font-medium text-xs sm:text-sm whitespace-nowrap transition-colors transition-transform duration-150 ${isActive
                      ...
                      <div className="space-y-6">
                        {settings.textProvider === 'huggingface' || settings.imageProvider === 'huggingface' ? (
                          <div>
                            ...
                            {/* Token Validation */}
                            {huggingFaceToken && (
                              <div className={`p-4 rounded-lg ${validateToken('huggingface', huggingFaceToken).valid
                                ...
                                <div className="flex items-center">
                                  ...
                                  <p className={`ml-2 text-sm ${validateToken('huggingface', huggingFaceToken).valid
                                      ? 'text-green-800'
                                      : 'text-yellow-800'
                                    }`}>
                        ...
                        {settings.textProvider === 'mistral' || settings.imageProvider === 'mistral' ? (
                          <div>
                            ...
                            {/* Key Validation */}
                            {mistralToken && (
                              <div className={`p-4 rounded-lg ${validateToken('mistral', mistralToken).valid
                                  ? 'bg-green-50 border border-green-200'
                                  : 'bg-yellow-50 border border-yellow-200'
                                }`}>
                                <div className="flex items-center">
                                  ...
                                  <p className={`ml-2 text-sm ${validateToken('mistral', mistralToken).valid
                                      ? 'text-green-800'
                                      : 'text-yellow-800'
                                    }`}>
